...
def command_phone_operations_check_decorator(func):
    def inner(*args, **kwargs) -> None:
        try:
            ...
            print('Тип аргумента неприйнятний!')
            return
        except ValueError:
            print(f'Забагато аргументів для {func.__name__}! Можливо, ви використовуєте забагато пробілів.')
            return
        except IndexError:
            print(f'Недостатньо аргументів для {func.__name__}!')
            return
        except TerribleException:
            print('''Щось ДІЙСТВІТЬЬНО невідоме трапилося під час читання вашої команди! Будь ласка, залиште
            спокій і виходьте з кімнати!''')
            return
        except KeyError:
...
class CommandOperation(ABC):

    @abstractmethod
    def execute(self, adr_book, *args, **kwargs):
        pass

class AddRecordOperation(CommandOperation):

    def execute(self, adr_book, line_list):
        if len(line_list) < 3:
...
class AddPhoneOperation(CommandOperation):
...
# More concrete command operation classes can be added similarly...
...
class Phone(Field):
    ...
    def value(self, new_value):
        ...
        if is_valid:
            self._value = self.convert_phone_number(new_value)
        else:
            print('Number format is not correct! Must contain 10-13 symbols and must match the one of the current '
                  'formats: +380001112233 or 80001112233 or 0001112233!')
            raise WrongArgumentFormat
...
def delete_record(adr_book, line_list):
    ...
    if line_list[1] in adr_book.data:
        ...
        print(f'{name.value} was successfully deleted!')
    else:
        print('No such name in the list!')


@command_phone_operations_check_decorator
def show_records(adr_book, line_list):
    if len(line_list) > 2:
        raise ExcessiveArguments
...
@command_phone_operations_check_decorator
def exit(adr_book, *args, **kwargs) -> bool:
    adr_book.is_finished = True
    return True


def command_phone_operations_check_decorator(command_function):
    def wrapper(*args, **kwargs):
        if args[1].is_finished:
            print('Command processing was canceled. Good bye!')
            exit()
...
command_list = {'hello': hello,
                ...
                'not save': close_without_saving,
                'bye': finish_session,
                'close': finish_session}

# command vocab with descriptions
command_description = {'hello': 'Hear some greeting from me',
                       ...
                       'bye': 'Save changes and close address book'}

def main():
    adr_book = AddressBook()

    print(
        '''Hello! I am your smart and useful command line interface for working with your record and phone book. You can 
        manipulate your records as you wish. But not literally. Actually, my suggestions are quite limited to basic phone
        operations. But don't you worry, it's actually way too enough for your phone book! Do you want to get some help? I
        can tell you all you want to know about all possible commands, my lord!''')

    while True:

        line = input()
        line_list = deconstruct_command(line)

        try:
            if line_list[0] not in command_list:
                raise TerribleException
            perform_command(line_list[0], adr_book, line_list)
        except TerribleException:
            print(
                f'Could not find a command like "{line_list[0]}". Check if the syntax is correct. You can also ask me '
                'about existing commands by typing "help"!')
...
class Application:
    ...
    def run(self):
        self.user_interface.display_message('Hello! Welcome to the address book application.')

        while True:
            self.user_interface.display_message('Enter a command:')
            ...
            try:
                if line_list[0] not in command_list:
                    raise TerribleException
                perform_command(line_list[0], self.address_book, line_list)
            except TerribleException:
                self.user_interface.display_message(f'Could not find a command like "{line_list[0]}". Check if the syntax is correct.')

            if self.address_book.is_finished:
                break

if __name__ == '__main__':
    console_interface = ConsoleInterface()
    application = Application(console_interface)
    application.run()
